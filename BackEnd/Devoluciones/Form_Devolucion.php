	...
	<body>
		...
		<div class="fondo123">
			<div class="container"> 
				...
				<H1>F O R M U L A R I O    D E V O L U C I O N E S </H1>
		<FORM ACTION= "insertDevolucion.php" METHOD="POST" class="formulario">
			<LABEL> Fecha </LABEL>
			<INPUT TYPE="date" NAME="fecha_devolucion"/>
			</br>
			</br>
			<LABEL> Salida </LABEL>
			<SELECT name="salida_devolucion">
				<?php
					header("Content-Type: text/html;charset=iso-8859-1");
					...
					if ($conexion)
					{
						...
						if ($resultado){
							while($row = sqlsrv_fetch_array($resultado,SQLSRV_FETCH_ASSOC))
							{
								...
								$refaccion_salida = $row ['refaccion_salida'];
								$cantidad_salida = $row ['cantidad_salida'];
								echo "<option value='".$folio_salida."'> #".$folio_salida." ".$refaccion_salida." con ".$cantidad_salida." unidades</option><br>";
							}
						}
					}
				?>
			...
			<LABEL> Almacen </LABEL>
			<SELECT name="almacen_devolucion">
				<?php
					header("Content-Type: text/html;charset=iso-8859-1");
					include("../conexion.php");
					if ($conexion)
					{
						...
						if ($resultado){
							while($row = sqlsrv_fetch_array($resultado,SQLSRV_FETCH_ASSOC))
							{
								...
								echo "<option value='".$id_almacen."'>".$descripcion."</option><br>";
							}
						}
					}
				?>	
