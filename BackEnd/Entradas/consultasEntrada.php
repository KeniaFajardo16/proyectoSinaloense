...
<html lang="es">
	...
	<div class="fondo123">
		<div class="container"> 
			...
			<H1> C O N S U L T A S    S A L I D A S </H1>
			...
			<?PHP
				...
				if ($conexion) {//si hubo conexion
					$resultado = sqlsrv_query($conexion, "exec spSeleccionarEntrada");
				...
				ECHO "<TABLE BORDER =1 frame=below id='tabla'>";
					...
					ECHO "<TH> Refaccion entrada </TH>";
					ECHO "<TH> Cantidad entrada </TH>";
					ECHO "<TH> Almacen entrada </TH>";
					...
					while ($i=$row=sqlsrv_fetch_array($resultado, SQLSRV_FETCH_ASSOC)){
						...
						$refaccion_entrada = $row ['refaccion_entrada'];
						$cantidad_entrada = $row ['cantidad_entrada'];
						$almacen_entrada = $row ['almacen_entrada'];
						
						//3.3 Imprimir resutados
						ECHO "<TR>";
							ECHO "<TD hidden>".$folio_entrada."</TD>";
							ECHO "<TD>".date_format($fecha_entrada, 'Y-m-d')."</TD>";
							ECHO "<TD>".$refaccion_entrada."</TD>";
							ECHO "<TD>".$cantidad_entrada." unidades</TD>";
							ECHO "<TD>".$almacen_entrada."</TD>";
							ECHO "<TD> <a href='form_modif.php?folio_entrada=".$folio_entrada ."&fecha_entrada=".date_format($fecha_entrada, 'Y-m-d')."&refaccion_entrada=".$refaccion_entrada."&cantidad_entrada=".$cantidad_entrada."&almacen_entrada=".$almacen_entrada."'><img src='../Sinaloense_files/modificar.png'></a></td>";
							echo "<TD> <a href='eliminarentrada.php?folio_entrada=".$folio_entrada."'><img src='../Sinaloense_files/eliminar.png'></a></td>";
						ECHO "</TR>";
					}
					ECHO "</TBODY>";
