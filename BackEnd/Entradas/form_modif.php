...
$folio_entrada =$_GET['folio_entrada'];
$fecha_entrada =$_GET['fecha_entrada'];
$refaccion_entrada =$_GET['refaccion_entrada'];
$cantidad_entrada =$_GET['cantidad_entrada'];
$almacen_entrada =$_GET['almacen_entrada'];
...
<html lang="es">
	...
	<div class="fondo123">
		<div class="container"> 
			...
			<form action = "modificarEntrada.php" METHOD="POST" class="formulario">
				<!--Cajas de texto !-->
					
				<input TYPE ="hidden" NAME = "folio_entrada" 
				value = <?php echo "".$folio_entrada."" ?> 
				/>
				</br>
					
				<label>Fecha Entrada </label>
				<INPUT type="date" value=<?php echo "'".$fecha_entrada."'" ?> name="fecha_entrada"/>
				</br>
				<label>Refaccion Entrada</label>
				<SELECT name="refaccion_entrada">
					<?php
						...
						if ($conexion)
						{
							...
							if ($resultado){
								while($row = sqlsrv_fetch_array($resultado,SQLSRV_FETCH_ASSOC))
								{
									...
									if($refaccion==$refaccion_entrada){
				...
				<input TYPE ="TEXT" NAME = "cantidad_entrada" 
				value = <?php 
				echo "".$cantidad_entrada."" ?> />
				</br>
				
				<label>Almacen Entrada</label>
				<SELECT name="almacen_entrada">
					<?php
						header("Content-Type: text/html;charset=iso-8859-1");
						include("../conexion.php");
						if ($conexion)
